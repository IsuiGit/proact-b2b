...
def record_general_feedback(text: str) -> list[dict]:
    """Record a free-form feedback line, extracting company/product hints where possible.

    If the text mentions companies (case-insensitive partial match against stored
    analyst/scout data), records per-company entries. Otherwise stores as a general
    note with rating guessed from sentiment keywords.
    """
    # Quick sentiment heuristic for rating
    text_lower = text.lower()
    positive_words = ["хорошо", "отлично", "нравится", "полезно", "правильно", "верно", "супер", "круто", "ок", "ок", "да", "верно"]
    negative_words = ["плохо", "неверно", "ошибка", "мусор", "хрень", "бесполезно", "не то", "нет", "зря", "ерунда"]
